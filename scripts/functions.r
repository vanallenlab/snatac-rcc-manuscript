...
create_motif_significance_plot <- function(enriched.motifs, 
                                           max_logp = 320, 
                                           n_annotate = 10, 
                                           specific_substr = NULL, 
                                           pval_threshold = 0.05, 
                                           low_color_bound = "lightblue", 
                                           high_color_bound = "blue", 
                                           color_gradient_limits = c(0, 2), 
                                           max_overlaps = Inf,
                                           use_unadjusted_pval = FALSE) {
  # Choose which p-value column to use
  pval_vector <- if (use_unadjusted_pval) enriched.motifs$pvalue else enriched.motifs$p.adjust
  pval_label <- if (use_unadjusted_pval) "pvalue" else "p.adjust"
  
  # Add chosen p-value column to the dataframe
  enriched.motifs$pval_to_use <- pval_vector
  df <- enriched.motifs %>%
    mutate(
      neg_log_pval = ifelse(pval_to_use == 0, max_logp, -log10(pval_to_use)),
      neg_log_pval = pmin(neg_log_pval, max_logp),
      rank = row_number(),
      significant = pval_to_use < pval_threshold,
      fold_enrichment_color = ifelse(significant, fold.enrichment, NA)
    )
  
  # Create the plot
  p <- ggplot(df, aes(x = rank, y = neg_log_pval)) +
    geom_point(aes(color = fold_enrichment_color), show.legend = TRUE) +
    scale_color_gradient(low = low_color_bound, high = high_color_bound,
                         limits = color_gradient_limits, na.value = "grey",
                         name = "Fold Enrichment") +
    labs(x = "Rank", y = paste0("-log10(", pval_label, ")"), 
         title = "Ranked TF Motif Significance") +
    theme_classic() +
  ...
  if (nrow(annotated_motifs) > 0) {
    p <- p + geom_text_repel(data = annotated_motifs, aes(label = motif.name),
...
write_hg19_bed <- function(obj, peaks, output_path) {
    ...
    granges$name = peaks
    path = "/home/jupyter/scATAC_analysis/edit/data/scripts/hg38ToHg19.over.chain"
